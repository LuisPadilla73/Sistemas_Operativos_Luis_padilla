...
//#else
//    id = (uint8_t)((id + 1U) % 3U);  // heart->temp->both->heart...
//    xQueueOverwriteFromISR(CurrentIDmailbox, &id, &xHPW);
//    GPIO_PortClearInterruptFlags(GPIOC, (1U << 6));
//#endif
//
...
int main(void)
{
   ...
   // GPIO_BOARD_INIT();
    ...
    PointQueueHR      = xQueueCreate(5, sizeof(uint16_t));
    PointQueueTEMP    = xQueueCreate(5, sizeof(uint16_t));
...
static void LCDprint_thread(void *pvParameters)
{
    ...
    /* Conserva el último valor visto por si no hay nada en el buzón en este ciclo */
    ...
    for (;;)
    {
        ...
        /* Lee cambios de modo; si cambió, limpia y reinicia plumas */
        if (xQueuePeek(CurrentIDmailbox, &mode, 0) == pdTRUE || mode != last_mode)
        {
            LCD_nokia_clear();
        ...
        if (mode == heart || mode == both)
        {
            if (xQueueReceive(PointQueueHR, &y_hr, 0) == pdTRUE)
            {
                a_hr = b_hr;
                b_hr.y = y_hr;
                if (b_hr.x + x_increment < 84) b_hr.x += x_increment;
                else                           b_hr.x  = 0;
        ...
        if (mode == temp || mode == both)
        {
            if (xQueueReceive(PointQueueTEMP, &y_tp, 0) == pdTRUE)
            {
                a_tp = b_tp;
                b_tp.y = y_tp;
                if (b_tp.x + x_increment < 84) b_tp.x += x_increment;
                else                           b_tp.x  = 0;
...
static void GraphProcess_thread(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        {
            if (m.convSource == 0) { /* HR */
                /* 0..4095 -> 0..23 */
                y = (uint16_t)((m.data * 24U) / 4096U);  // top half
                xQueueSend(PointQueueHR, &y, portMAX_DELAY);
            } else {                 /* TEMP */
                /* 0..4095 -> 0..23 y luego desplaza +24 */
                y = (uint16_t)((m.data * 24U) / 4096U);
                y = (uint16_t)(y + 24U);                 // bottom half
                xQueueSend(PointQueueTEMP, &y, portMAX_DELAY);
            }
            taskYIELD();
        }
    }
